...
_STATE_FILE = Path("logs/web_state.json")
...
def _persist_state() -> None:
    try:
        ...
        _STATE_FILE.write_text(json.dumps(snapshot, ensure_ascii=False, indent=2), encoding="utf-8")
    except Exception:
        pass
...
def pipeline_worker(mode: str, seed_url: str | None, trigger: str) -> None:
    ...
    try:
        ...
        append_state_output("\n" + error_trace)
        _save_error_log(mode, trigger, exc, error_trace)
        set_state(
            status="error",
            running=False,
            last_finished_at=now_iso(),
...
def _save_error_log(mode: str, trigger: str, exc: Exception, trace: str) -> None:
    try:
        log_dir = Path(botfactory_main.getenv_str("LOGS_DIR", "logs"))
        log_dir.mkdir(parents=True, exist_ok=True)
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        log_path = log_dir / f"pipeline_error_{ts}.log"
        log_path.write_text(
            f"timestamp: {now_iso()}\nmode: {mode}\ntrigger: {trigger}\nerror: {exc}\n\n{trace}",
            encoding="utf-8",
        )
